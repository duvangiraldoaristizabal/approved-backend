...
CREATE TABLE IF NOT EXISTS approval_requests (
  ...
  type VARCHAR(30) NOT NULL CHECK (type IN ('DEPLOYMENT', 'ACCESS', 'TECHNICAL_CHANGE', 'TOOL_ONBOARDING', 'OTHER')),
...
CREATE INDEX IF NOT EXISTS idx_requests_approver_status ON approval_requests(approver, status);
CREATE INDEX IF NOT EXISTS idx_requests_requester ON approval_requests(requester);
CREATE INDEX IF NOT EXISTS idx_history_request_changed ON approval_history(request_id, changed_at);
`;
